...
class YoutubeFollower():
    ...
    def get_recommendations(self, video_id, nb_recos_wanted, depth, key):
        ...
        video = self._video_infos[video_id]
        print(video_id + ': ' + video['title'] + ' [' + channel + ']{' + repr(key) + '} ' + str(
            video['views']) + ' views , recommendations:' + repr(len(recos)))
...
def compare_keywords(query, search_results, branching, depth, name, gl, language, recent, loopok, alltime):
    ...
    for keyword in query.split(','):
        yf = YoutubeFollower(verbose=True, name=keyword, alltime=alltime, gl=gl,
                             language=language, recent=recent, loopok=loopok)
...
def main():
    ...
    compare_keywords(args.query, args.searches, args.branch, args.depth, args.name,
                     args.gl, args.language, args.recent, args.loopok, args.alltime)
    return 0
